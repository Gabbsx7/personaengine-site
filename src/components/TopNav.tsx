    ...
    <header className="fixed top-0 inset-x-0 z-50 bg-black/60 backdrop-blur-md border-b border-white/10 h-16 flex items-center">
      <div className="max-w-6xl mx-auto flex items-center justify-between px-6 w-full h-full">
        <Link href={logoHref} className="flex items-center gap-4 hover:opacity-90 h-full">
          <img src="/personaengine-logo.svg" alt="Persona Engine logo" className="h-18 w-18 -mt-2" style={{ height: 72, width: 72 }} />
          <span className="text-lg font-black flex items-center h-full">Persona <span className="text-yellow-400">Engine</span></span>
        </Link>
          ...
          {navItems.map(item =>
            item.href ? (
              <Link key={item.id} href={item.href} className="relative group">
                {item.label}
                <span className="absolute left-0 -bottom-1 h-0.5 w-0 bg-yellow-400 group-hover:w-full transition-all" />
              </Link>
            ) : (
